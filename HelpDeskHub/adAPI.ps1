...
Stop-Process -Id (Get-Content "C:\Automation\AutoServerMonitor\adAPI--PID.txt") -ErrorAction SilentlyContinue
$PID | Set-Content C:\Automation\AutoServerMonitor\adAPI--PID.txt -Force
$host.UI.RawUI.WindowTitle = "adAPI"

$listener = New-Object System.Net.HttpListener
#$apiPort = Get-Content \\fileshare01\home\dpoe\CRM\adApiPort.txt
#$apiPort = Get-Content c:\Automation\HelpDeskHub\adApiPort.txt
$apiPort = 1322
...
$exSession = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://i-mail.is-u.jrtc.army.mil/PowerShell/
Import-PSSession $exSession -DisableNameChecking -CommandName Enable-Mailbox,Disable-Mailbox | Out-Null
$lySession = New-PSSession -ConnectionUri https://i-skype.is-u.jrtc.army.mil/OcsPowershell -Authentication NegotiateWithImplicitCredential
Import-PSSession $lySession -DisableNameChecking -CommandName Get-CsUser,Enable-CsUser,Disable-CsUser,Set-CsUser | Out-Null

while($true){
  #$apiKey = Get-Content \\fileshare01\home\dpoe\CRM\adApiKey.txt
  #$apiKey = Get-Content c:\Automation\HelpDeskHub\adApiKey.txt
  ...
  "AD__API  $((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: $requester -- $request" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_web_log.txt
  ...
  if($request[0] -eq $apiKey){
    switch($request[1]){
      ...
      "pwdRSub" {
        ...
        elseIf($pwd -eq $newPwd){
          ...
          if($pCheck){
            ...
            elseIf($pCheck){
              "$((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: Password Reset - $user" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_actions_log.txt
      ...
      "abBuild" {
        ...
        $buildData.pwd = Get-Content \\i-file\software$\RCS\Derek.Poe\Reports\reportData\pdb208ffn38gfb38fba9ff.txt | ConvertTo-SecureString -Key (Get-Content \\i-file\software$\RCS\Derek.Poe\Reports\reportData\b1n9rk322ndh29dn39d.key)
        $pwdDec = "Default"

        switch($buildData.group){
          "Task Force 1" {
            $ouSel = "OU=TF_1,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TF1 TAFF", "TAF User")
            break
          }
          "Task Force 2" {
            $ouSel = "OU=TF_2,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force 3" {
            $ouSel = "OU=TF_3,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force 4" {
            $ouSel = "OU=TF_4,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force 5" {
            $ouSel = "OU=TF_5,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force Sustainment" {
            $ouSel = "OU=TF_SUST,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force Aviation" {
            $ouSel = "OU=TF_AVN,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force BMC" {
            $ouSel = "OU=TF_BMC,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force FireSupport" {
            $ouSel = "OU=TF_FS,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Task Force SOTD" {
            $ouSel = "OU=TF_SOTD,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "OpFor" {
            $ouSel = "OU=OPFOR,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "Valiant" {
            $ouSel = "OU=Valiant,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "TAF User")
            break
          }
          "LiveFire" {
            $ouSel = "OU=LiveFire,OU=TASKFORCES,OU=Users,OU=Enterprise Users,DC=is-u,DC=jrtc,DC=army,DC=mil"
            $grSel = @("Domain Users", "SG_LIVEFIRE")
            break
          }
        }

        New-ADUser -AccountExpirationDate $buildData.expDate -AccountPassword $buildData.pwd -DisplayName $buildData.dname -GivenName $buildData.fname -Initials $buildData.mi -Surname $buildData.lname -SamAccountName $buildData.logon -Name $buildData.dname -Enabled $buildData.enabled -UserPrincipalName "$($buildData.logon)@is-u.jrtc.army.mil" -PasswordNeverExpires $false -SmartcardLogonRequired $buildData.smartcard -Description $buildData.description
        ...
        waitMB
        $enLY = enable-csuser -identity $user -registrarpool "i-skype.is-u.jrtc.army.mil" -sipaddresstype emailaddress -sipdomain is-u.jrtc.army.mil
        $null = set-csuser -identity $user -enterprisevoiceenabled $true
        verifyMBLY
        ...
        "$((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: Account Created - $($buildData.logon)" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_actions_log.txt
        break
      }
      ...
      "abPwdSet" {
        ...
        "$((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: Password Set - $($setData[0])" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_actions_log.txt
        break
      }
      ...
      "amManage" {
        ...
        forEach($prop in $manInfo.PSObject.Properties.Name){
          switch($prop){
            "NewLogon"{
              $set = Set-ADUser -Identity $manInfo.Logon -UserPrincipalName "$($manInfo.NewLogon)@is-u.jrtc.army.mil" -SamAccountName $manInfo.NewLogon
              break
            }
            "Enabled"{
              if($manInfo.Enabled){
        ...
        "$((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: Account Modified - $($manInfo.Logon) - $(($manInfo.PSObject.Properties.Name  | ? {$_ -ne "Logon"}) -join ",")" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_actions_log.txt
        break
      }
      ...
      "amDelete" {
        ...
        "$((Get-Date).ToString("MM/dd/yyyy hh:mm:ss tt")):: Account Deleted - $user" | Add-Content -Force -Path c:\Automation\HelpDeskHub\ad_actions_log.txt
        break
      }

      "getFirstName" {
        $byteMess = $enc.GetBytes(((Get-ADUser ($request[2])).GivenName))
        $context.Response.OutputStream.Write($byteMess,0,$byteMess.Length)
        $context.Response.Close()
        break
      }
